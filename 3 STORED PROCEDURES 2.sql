...
-- 2 NEW COLUMNS ADDED RELEVANT TO QUARENTINE 
...
/////MINOR TWEAK IN THE ABOVE RETRY SP 
//Created a box to hold it: v_sqlstate VARCHAR;
//Put the error code in the box, then used the box: v_sqlstate := SQLSTATE; ...then passed v_sqlstate instead of SQLSTATE.
  ...
  BEGIN
    FOR rec IN cr_rules DO
      ...
      ELSE
        ...
        LET v_quarantine_tbl VARCHAR := NULL;
        LET v_rows_quarantined NUMBER := 0;

        IF (:v_action = 'QUARANTINE') THEN
          SELECT QUARANTINE_TABLE INTO v_quarantine_tbl
          FROM NEW_TEST.REFERENCE.QUARANTINE_REGISTRY
          WHERE PIPELINE_CODE = :P_PIPELINE_CODE AND IS_ACTIVE = TRUE;

          IF (:v_quarantine_tbl IS NOT NULL AND :v_col_name IS NOT NULL) THEN
            LET v_quarantine_sql VARCHAR;
            IF (:v_rule_type = 'NOT_NULL') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — ' || :v_col_name || ' IS NULL'' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE src.' || :v_col_name || ' IS NULL';
            ELSEIF (:v_rule_type = 'UNIQUE') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — duplicate ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE src.' || :v_col_name ||
                ' IN (SELECT ' || :v_col_name || ' FROM ' || :v_target_table || ' GROUP BY ' || :v_col_name || ' HAVING COUNT(*) > 1)';
            ELSEIF (:v_rule_type = 'ACCEPTED_VALUES') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — invalid value in ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE src.' || :v_col_name ||
                ' NOT IN (''' || REPLACE(:v_expression, ',', ''',''') || ''') AND src.' || :v_col_name || ' IS NOT NULL';
            ELSEIF (:v_rule_type = 'REGEX') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — regex fail in ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE NOT REGEXP_LIKE(src.' || :v_col_name ||
                ', ''' || :v_expression || ''') AND src.' || :v_col_name || ' IS NOT NULL';
            ELSEIF (:v_rule_type = 'RANGE') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — out of range in ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE (src.' || :v_col_name ||
                ' < ' || SPLIT_PART(:v_expression, ',', 1) || ' OR src.' || :v_col_name || ' > ' || SPLIT_PART(:v_expression, ',', 2) ||
                ') AND src.' || :v_col_name || ' IS NOT NULL';
            ELSEIF (:v_rule_type = 'REFERENTIAL') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — referential integrity fail in ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE src.' || :v_col_name ||
                ' NOT IN (SELECT ' || SPLIT_PART(:v_expression, '.', 4) || ' FROM ' ||
                SPLIT_PART(:v_expression, '.', 1) || '.' || SPLIT_PART(:v_expression, '.', 2) || '.' || SPLIT_PART(:v_expression, '.', 3) ||
                ') AND src.' || :v_col_name || ' IS NOT NULL';
            END IF;

            IF (:v_quarantine_sql IS NOT NULL) THEN
              EXECUTE IMMEDIATE :v_quarantine_sql;
              v_rows_quarantined := :v_rows_failed;
            END IF;
          END IF;
        END IF;

        INSERT INTO NEW_TEST.AUDIT.DQM_CHECK_LOG (RUN_ID, ENVIRONMENT, PIPELINE_CODE, PIPELINE_LAYER, TARGET_TABLE, DOMAIN, RULE_CODE, RULE_NAME, RULE_TYPE, COLUMN_NAME, RULE_EXPRESSION, SEVERITY, CHECK_STATUS, ROWS_CHECKED, ROWS_PASSED, ROWS_FAILED, ROWS_QUARANTINED, FAILURE_RATE_PCT, VIOLATION_SAMPLE, ACTION_ON_FAIL, ACTION_TAKEN, QUARANTINE_TABLE, EXECUTION_MS)
        VALUES (:P_RUN_ID, :P_ENVIRONMENT, :P_PIPELINE_CODE, :v_layer, :v_target_table, :v_domain, :v_rule_code, :v_rule_name, :v_rule_type, :v_col_name, :v_expression, :v_severity, 'FAILED', :v_rows_checked, :v_rows_checked - :v_rows_failed, :v_rows_failed, :v_rows_quarantined, :v_fail_pct, :v_rows_failed || ' failures in ' || :v_col_name || ' (' || :v_rule_type || ')', :v_action, :v_action, :v_quarantine_tbl, :exec_ms);
      END IF;
    END FOR;
  END;
...
//Corrected add one pk column logic ensure qurantine tables aren't duplicated
  ...
  BEGIN
    FOR rec IN cr_rules DO
      ...
      ELSE
        ...
        IF (:v_severity = 'CRITICAL') THEN v_critical_failures := v_critical_failures + 1; END IF;
        LET v_quarantine_tbl VARCHAR := NULL;
        LET v_pk_column VARCHAR := NULL;                              -- ★ PK from registry (dynamic)
        LET v_rows_quarantined NUMBER := 0;
        IF (:v_action = 'QUARANTINE') THEN
          -- ★ read BOTH the quarantine table AND the stable PK column from the registry
          SELECT QUARANTINE_TABLE, PK_COLUMN INTO v_quarantine_tbl, v_pk_column
          FROM NEW_TEST.REFERENCE.QUARANTINE_REGISTRY
          WHERE PIPELINE_CODE = :P_PIPELINE_CODE AND IS_ACTIVE = TRUE;
          IF (:v_quarantine_tbl IS NOT NULL AND :v_col_name IS NOT NULL) THEN
            LET v_quarantine_sql VARCHAR;
            -- ★ DEDUP GUARD: only quarantine a bad row if it is NOT already in
            --    quarantine for THIS rule (matched on the registry PK column).
            --    Blocks duplicate rows on every rerun. Resolved (REPROCESSED)
            --    rows are deleted, so a row that goes bad again can re-enter.
            LET v_dedup VARCHAR :=
              ' AND NOT EXISTS (SELECT 1 FROM ' || :v_quarantine_tbl || ' q' ||
              ' WHERE q.' || :v_pk_column || ' = src.' || :v_pk_column ||
              ' AND q.QUARANTINE_RULE_CODE = ''' || :v_rule_code || '''' ||
              ' AND q.QUARANTINE_STATUS IN (''PENDING'',''RETRY_FAILED'',''REJECTED''))';
            IF (:v_rule_type = 'NOT_NULL') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — ' || :v_col_name || ' IS NULL'' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE src.' || :v_col_name || ' IS NULL' || :v_dedup;
            ELSEIF (:v_rule_type = 'UNIQUE') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — duplicate ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE src.' || :v_col_name ||
                ' IN (SELECT ' || :v_col_name || ' FROM ' || :v_target_table || ' GROUP BY ' || :v_col_name || ' HAVING COUNT(*) > 1)' || :v_dedup;
            ELSEIF (:v_rule_type = 'ACCEPTED_VALUES') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — invalid value in ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE src.' || :v_col_name ||
                ' NOT IN (''' || REPLACE(:v_expression, ',', ''',''') || ''') AND src.' || :v_col_name || ' IS NOT NULL' || :v_dedup;
            ELSEIF (:v_rule_type = 'REGEX') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — regex fail in ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE NOT REGEXP_LIKE(src.' || :v_col_name ||
                ', ''' || :v_expression || ''') AND src.' || :v_col_name || ' IS NOT NULL' || :v_dedup;
            ELSEIF (:v_rule_type = 'RANGE') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — out of range in ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
                :P_RUN_ID || ''', ''PENDING'', 0, NULL, NULL FROM ' || :v_target_table || ' src WHERE (src.' || :v_col_name ||
                ' < ' || SPLIT_PART(:v_expression, ',', 1) || ' OR src.' || :v_col_name || ' > ' || SPLIT_PART(:v_expression, ',', 2) ||
                ') AND src.' || :v_col_name || ' IS NOT NULL' || :v_dedup;
            ELSEIF (:v_rule_type = 'REFERENTIAL') THEN
              v_quarantine_sql := 'INSERT INTO ' || :v_quarantine_tbl ||
                ' SELECT UUID_STRING(), src.*, ''' || :v_rule_name || ' — referential integrity fail in ' || :v_col_name || ''' AS QUARANTINE_REASON, ''' ||
                :v_rule_code || ''' AS QUARANTINE_RULE_CODE, ''' || :v_severity || ''' AS QUARANTINE_SEVERITY, CURRENT_TIMESTAMP(), NULL, ''' ||
